...
6. **Testes de verdade.** Carregue a skill `testador-real` (ferramenta Skill — invoque, não descreva; ou o testador do projeto); ela executa as baterias estática + dinâmica. *Gate: relatório datado sem FAIL crítico aberto.*
7. **Segurança.** Carregue a skill `especialista-seguranca` (ferramenta Skill — invoque, não descreva); ela revisa superfície, autenticação, dados sensíveis (LGPD) — obrigatório antes de expor qualquer coisa publicamente. *Gate: sem achado crítico/alto aberto.*
