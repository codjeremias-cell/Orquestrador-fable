...
description: "Transforma ideia ou pedido vago em requisitos acionáveis: problema e público, escopo com corte de MVP, histórias com critérios de aceite verificáveis, requisitos não funcionais mensuráveis, riscos e premissas. Acione também sem pedido explícito quando a conversa desenhar tela, banco ou stack sem escopo escrito. Acione com \"quero fazer um sistema/app que...\", \"me ajuda a tirar essa ideia do papel\", \"o que esse sistema precisa ter?\", \"levanta os requisitos\", \"define o escopo/MVP\", \"seria bom ter um app pra...\", \"ainda não sei bem o que quero, mas...\", \"como eu começo esse projeto?\", \"isso aqui vale virar software?\". NÃO acione para avaliar viabilidade comercial/monetização (use consultor-negocios-apps), para decidir a estrutura técnica (use arquiteto-software), nem para melhorar o que JÁ existe — retrabalho, processo manual repetitivo (use inovacao-melhorias, que vem ANTES) — esta skill define O QUE construir e por quê; as outras decidem se vale e como."
---
...
- **Vem antes:** `consultor-negocios-apps` quando a dúvida é "vale a pena?" — o parecer de negócio alimenta o corte de escopo · `inovacao-melhorias` quando o pedido descreve algo que **já existe** e incomoda (retrabalho, processo manual, desperdício): ela decide *se* vale melhorar e com que hipótese, e só a oportunidade aprovada vira escopo aqui. A `inovacao-melhorias` declara esta skill como o "vem depois" dela — o par é uma **ordem**, não uma escolha.
...
### 📜 Histórico
- **2026-09-22 — A fronteira que faltava: esta skill estava pegando o caso da `inovacao-melhorias` (medição de acionamento de 2026-09-22, `_auditoria/zelador-inventario-2026-09-22.md`; RI-04; degrau §6.10: 1 — só edição).** Medido em sessão nova, com prompt neutro e **artefato real** na pasta (um relatório mensal com *"copiar coluna A"*, *"conferir à mão"*, *"repetir para 40 lojas"*): quem escreveu *"todo mês a gente refaz o mesmo relatório na mão e sempre dá erro"* recebeu **esta** skill, não a `inovacao-melhorias`. **Não é instância:** o mesmo resultado saiu em 2026-08-11 e de novo em 2026-09-22, com 42 dias entre as duas medições. O efeito é que a etapa de *"vale melhorar isto, com que hipótese e como medir"* **é pulada**, e o usuário vai direto para "vamos especificar o que construir". A causa não é a vizinha ser fraca — é esta `description` não ter fronteira nenhuma para ela: a cláusula `NÃO acione` nomeava `consultor-negocios-apps` e `arquiteto-software` e **não mencionava a `inovacao-melhorias` em lugar algum**. Entrou a cláusula que devolve o caso à dona, mais a linha de ordem no "Vem antes" da Rede. Para caber no teto de 1024 saiu a frase-gatilho *"preciso de um sistema que..."*, que repetia o ramo de *"quero fazer um sistema/app que..."* — 983 caracteres, folga de 41. **Limite declarado:** o efeito da fronteira **ainda não foi remedido**; a remedição é o próximo ato. **Modificadores de obrigatoriedade auditados (PADRÃO §12): N = 0** — o que entrou foi fronteira (`NÃO acione`) e ordem no "Vem antes"; as duas **endurecem** o roteamento, nenhuma abranda. **Efeito colateral declarado:** a `description` saiu de 904 para **983 caracteres** e passou a disparar o aviso do validador (96% do teto; antes não disparava). Foi **aceito, não consertado**: a saída óbvia — podar mais frases-gatilho — é justamente a que o Modo 3 do `zelador-do-catalogo` desaconselha, porque sinônimo sustenta o nível neutro (157/159 medidos) e o defeito aqui era **falta de fronteira**, não excesso de vocabulário. O aviso significa "a próxima frase-gatilho só entra se abrir ramo novo", e é exatamente o regime correto para esta skill agora.
