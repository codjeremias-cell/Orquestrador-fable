...
- **Teste de Reversibilidade Operacional para Agentes Autônomos (Garimpo 2026-09):** O raio de impacto define o grau de autonomia permissível:
  - *Ações Reversíveis (Autonomia Plena):* Leituras, pesquisas, inspeções, edições locais e execução de testes em ambiente sandbox têm autonomia total — investigue e teste livremente sem pedir permissão para cada leitura.
  - *Ações Irreversíveis (Barreira Mandatória de AUTH):* Pushes para repositórios remotos, merges em branches de produção, publicação de pacotes/releases, exclusão de dados/tabelas e disparos reais de comunicação (WhatsApp, e-mails, webhooks) são terminantemente proibidos sem autorização prévia explícita e evidência verificada. Na dúvida se algo é irreversível, **pare e espere**.


## Regras Inquebráveis (RI) — esta lente as faz cumprir
> Redação canônica e fonte única: governança `REGRAS-DE-OURO.md` — a versão vigente é a que está **na fonte**, não aqui (RI-01: confira, não lembre). Aqui, cada RI leva o gloss fiel + o delta operacional do gate.
...
### 📜 Histórico
- **2026-09-22 — Citação de versão retirada da fonte única (inventário `_auditoria/zelador-inventario-2026-09-22.md`, veredito `ATUALIZAR` mantido; RI-04; degrau §6.10: 1 — só edição).** A abertura das Regras Inquebráveis citava a governança como **`v2.8`** e o arquivo canônico está em **`v2.10`** desde 2026-08-19 (`REGRAS-DE-OURO.md` L6, conferido antes de editar) — alegação viva e errada **sobre a própria régua que esta lente faz cumprir**, e a mais cara de todas de se errar aqui. **O conserto não foi trocar `v2.8` por `v2.10`:** esse envelhece de novo na próxima revisão da fonte. O número **saiu**, e ficou o que não envelhece — a fonte nomeada mais a RI-01, que manda conferir em vez de lembrar. É o mesmo conserto que a `memoria-de-projeto` recebeu em **2026-08-11**, na mesma rodada de inventário que abriu este `ATUALIZAR`: as duas tinham o defeito idêntico, uma foi consertada de modo durável e esta ficou com a prescrição que decai. **Limite declarado, e ele continua aberto:** o `evals/placar-baseline.md` segue `PENDING — medição nunca executada` desde 2026-07-23, então o gate do §11 do `PADRAO-DE-AUTORIA` permanece aberto e o Selo Lendário segue não declarável. Isso é nível OBEDIÊNCIA, não mérito, e não foi rodado aqui. **Modificadores de obrigatoriedade auditados (PADRÃO §12): N = 0** — a linha que entrou é imperativa (*"confira, não lembre"*), sem "pode", "opcional" ou "se quiser".
