...
- **PROCEDIMENTO OPERACIONAL PADRÃO (SOP / POP):** para rotinas operacionais de equipe, incidentes, suporte e deploys manuais — estruturado com objetivo, escopo, pré-requisitos, passo a passo tabular com ação atômica e evidência, matriz de contingência/rollback e SLAs. Modelo canônico em [referencia/template-sop-operacional.md](referencia/template-sop-operacional.md).
...
4. **Testar o que o documento afirma:** cada comando de instalação/execução do README é executado antes de entrar (evidência — RI-04). Exemplo de código que o leitor copia deve nascer em teste ou exemplo compilável e ser injetado, ou comparado mecanicamente, com a documentação; delimitador ausente ou invertido é falha, nunca sincronização silenciosa. O que não puder testar, marcar "não verificado".
...
- **2026-08-29 — Snippet copiável com fonte executável (garimpo linear 2026-08-29 · G3; degrau §6.10: 1 — só edição).** O fluxo passou a exigir que exemplos de código destinados a cópia nasçam em teste/exemplo compilável e sejam injetados ou comparados mecanicamente com a documentação; marcador ausente ou invertido falha fechado. A regra é uma consolidação pequena sobre a exigência anterior de testar comandos e sobre o TWINS código ↔ documentação do `testador-real`. Modificadores de obrigatoriedade auditados (PADRÃO §12): N = 0. Proveniência: `linear/linear@fed840a976a755c8a9e5ecb6948f66307114e88c` — laudo em `garimpo-linear-2026-08-29.md`.

- **2026-08-19 — Garimpo skills-ia (G7; degrau §6.10: 2 — referência complementar; N = 0):** adicionado modelo canônico de Procedimento Operacional Padrão [referencia/template-sop-operacional.md](referencia/template-sop-operacional.md) para rotinas operacionais de equipe, deploys manuais e processos de suporte com tratamento de falha e SLAs. Proveniência: `skills-ia` de `github.com/tiagopgr/skills-ia` @ `1983bef` (MIT) — laudo em `garimpo-skills-ia-2026-08-19.md`.
