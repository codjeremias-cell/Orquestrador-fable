...
### 📜 Histórico
- **2026-07-20 — Polimento de autoria (boas práticas SKILL.md):** `description` com a fronteira entre as três lentes irmãs explícita no fim (modelo/evolução aqui · micro-tuning/DAO = dev-senior · macro não-dados = arquiteto-software) e caixa-alta normalizada; campo `when_to_use` com frases-gatilho extras (consolidado na `description` na compressão de 2026-08-08); checklist RI-04 promovido a seção **Verificação** com o *porquê* (autocheck antes do handoff, já que erro sobre dado em produção não tem "desfazer"); "expand/contract" ganhou a razão inline. Conteúdo, exemplos e salvaguardas preservados.
