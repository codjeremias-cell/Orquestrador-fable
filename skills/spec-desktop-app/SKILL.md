...
3. **Arquitetura + modelo de dados.** Carregue as skills `arquiteto-software` (limite front×core, IPC) e `arquiteto-dados` (ferramenta Skill — invoque, não descreva)
   (modelo SQLite local, migrações, chaves). Em seguida carregue `especialista-seguranca` (ferramenta Skill — invoque, não descreva) para **auditar o esquema enquanto
   ele ainda é barato de mudar**: campo temporal que o processo confiável deveria carimbar, campo que o usuário não pode reescrever, e o que ancora a posse do registro.
   *Gate: ADRs + esquema inicial; toda origem-de-verdade de campo temporal ou de posse **decidida aqui**, não no passo 5.*
...
### 📜 Histórico
- **2026-08-19 — a lente de segurança entra no passo 3, onde a decisão ainda é barata (T45; degrau §6.10: 1 — só edição).** Propagado do achado medido na `spec-mobile-app`, e **aqui não houve medição própria** — o mecanismo é que se aplica, não o número. Lá, nas duas rodadas tratadas da bancada T15, o `especialista-seguranca` foi **invocado** e as *security rules* **empataram** com o braço que não o invocou; provou-se, por comparação interna ao próprio braço, que **a lente ratifica a decisão de design e não a determina** — sobre a mesma ameaça ela escreveu *"mitigado nas rules"* onde o design já carimbava o horário no servidor e *"baixo, trade-off aceito"* onde o cliente escolhia o próprio timestamp. Causa: ela chegava depois, com o esquema já nascido. Aqui o intervalo era menor (passo 3 → 5) mas de mesma natureza: o `arquiteto-dados` desenhava o esquema SQLite e a lente só olhava capabilities e CSP adiante. Agora ela audita o esquema **junto** com quem o desenha, e o gate exige origem-de-verdade de campo temporal e de posse decidida ali. **Modificadores de obrigatoriedade auditados (PADRÃO §12): N = 0.**
