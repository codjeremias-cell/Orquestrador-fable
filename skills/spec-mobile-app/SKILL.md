...
8. **Testes de verdade.** Carregue a skill `testador-real` (ferramenta Skill — invoque, não descreva); ela executa estática (`flutter analyze`, `flutter test`, `dart run build_runner`) + dinâmica (`integration_test`/app no emulador). *Gate: relatório datado sem FAIL crítico aberto.*
9. **Segurança.** Carregue a skill `especialista-seguranca` (ferramenta Skill — invoque, não descreva); ela revisa rules, Auth (anti-enumeração), dados sensíveis (LGPD) e **App Check** antes de publicar. *Gate: sem achado crítico/alto aberto.*
