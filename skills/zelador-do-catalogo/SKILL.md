...
description: "Mede a saúde do próprio acervo de skills em três modos: CUSTO (contexto permanente por componente, com cortes ranqueados), INVENTÁRIO (manter, melhorar, aposentar ou fundir, com motivo autossuficiente) e OBEDIÊNCIA (a skill é seguida com prompt neutro?). Também antes de adicionar skill nova, para saber se há espaço. Acione com \"audita o catálogo\", \"quanto isso custa de contexto?\", \"quais skills estão obsoletas?\", \"essa skill está sendo obedecida?\", \"tem skill duplicada aqui?\", \"minha janela de contexto tá pesada\", \"o catálogo tá inchado?\", \"quantos tokens as descriptions comem?\", \"essa skill ainda serve?\", \"a skill dispara sozinha?\", \"o agente seguiu a skill até o fim?\". NÃO acione para auditar UMA entrega ou o processo de um trabalho (use auditor-responsabilidades), para propor melhoria de produto (inovacao-melhorias), nem para validar estrutura de arquivo — isso é o validar-skills.ps1, que este chama."
---
...
### 📜 Histórico
- **2026-09-22 — `description` abaixo do teto, e o achado que a abriu era falso (inventário `_auditoria/zelador-inventario-2026-09-22.md`, veredito `MELHORAR`; degrau §6.10: 1 — só edição).** Duas coisas, e a primeira **derruba** o motivo pelo qual esta skill estava em `ATUALIZAR`: a C-65 acusava **ponteiro morto** na Rede — a citação de `especialista-seguranca/referencia/seguranca-agentica.md` — e **o arquivo existe no disco**, conferido hoje. Achado falso, nada a consertar; a própria linha do inventário anterior já registrava a dúvida em `estrutura.obs`. O que restou veio de **instrumento independente**, não da opinião desta skill sobre si mesma: o `validar-skills.ps1` acusava `description` com **998 de 1024 caracteres** (folga de 26), e o PADRÃO §4.6 manda que a próxima frase-gatilho só entre se abrir ramo novo — com 26 de folga, nenhuma entra. Saíram **duas** frases redundantes — *"o que dá pra aposentar?"* e *"dá pra fundir essas duas?"* —, escolhidas porque os verbos **aposentar** e **fundir** continuam no texto, na frase que descreve o modo INVENTÁRIO: o ramo lexical não se perdeu, só a repetição. **938 caracteres, folga de 86.** Corte conservador de propósito: o §6 desta própria skill mede que sinônimo sustenta o nível neutro (157/159), então cortar vocabulário tem preço — e o preço destas duas é zero porque o termo sobrevive. **Limite declarado:** o efeito do corte no disparo **não foi medido**; isso é Modo 3, e esta entrada é o registro de que ele está em aberto. **Autojulgamento declarado:** esta rodada julgou o zelador com o zelador; por isso o único achado que entrou veio de fora (o validador) e o que foi derrubado é fato de disco. **Modificadores de obrigatoriedade auditados (PADRÃO §12): N = 0** — a edição só **removeu** duas frases-gatilho da `description`; nenhum texto normativo mudou.
